...
- This directory is the final source-only snapshot prepared for submission.
- It includes the source code, configs, scripts, tests, and dependency list used by the final project.
- Datasets, checkpoints, logs, and other runtime artifacts are intentionally excluded.
- `PointNet++` and `DGCNN` are the completed report-facing methods in this source snapshot.
